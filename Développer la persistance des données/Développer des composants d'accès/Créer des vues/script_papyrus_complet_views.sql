...
SELECT COUNT(numcom) AS nb_com, COUNT(DISTINCT numfou) AS nb_fou
FROM entcom;
...
SELECT entcom.numcom, fournis.nomfou, libart, qtecde*priuni AS sous_tot, obscom
FROM fournis
...
WHERE prix1 < (SELECT MIN(prix1,prix2,prix3) 
               FROM vente 
			   JOIN produit ON vente.codart = produit.codart
			   WHERE libart LIKE "r%")
...
SELECT produit.codart, vente.numfou
FROM produit
...
SELECT vente.numfou, produit.codart
FROM produit
JOIN vente ON produit.codart = vente.codart
...
SELECT vente.numfou, SUM(stkphy) AS stock
FROM produit
JOIN vente ON produit.codart = vente.codart
...
SELECT produit.codart, qtecde*100/qteann AS ratio
FROM produit
JOIN ligcom ON produit.codart = ligcom.codart
WHERE qtecde > qteann*90/100;

--19 Pour l'année 2018

SELECT SUM(qtecde*(priuni + priuni*20/100)) AS CA_2018
FROM ligcom
JOIN entcom ON ligcom.numcom = entcom.numcom
WHERE YEAR(datcom) = "2018";

--20 Il faut vérifier si chaque combinaison NUMFOU+CODART de la table vente a son correspondant dans les tables ligcom + entcom. On procède à un SELECT pour cela.
SELECT vente.codart, vente.numfou 
FROM vente 
JOIN produit ON vente.codart = produit.codart
JOIN ligcom ON produit.codart = ligcom.codart
JOIN entcom ON ligcom.numcom = entcom.numcom
WHERE (vente.codart, vente.numfou) != (ligcom.codart, entcom.numfou);

--IV Besoins de mise à jour
...
--EXERCICE 2 : vues sur PAPYRUS

--1

CREATE VIEW v_GlobalCde
AS
SELECT produit.codart, SUM(qtecde) AS QteTot, SUM(qtecde)*priuni AS PrixTot
FROM ligcom
JOIN produit
