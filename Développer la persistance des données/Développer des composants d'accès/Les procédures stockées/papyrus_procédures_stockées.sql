...
CREATE PROCEDURE CA_fournisseur(
	In codefournis varchar(25),
	In annee int(4)
	)
BEGIN

SELECT SUM(qtecde*(priuni + priuni*20/100)) AS CA_annee
FROM ligcom
JOIN entcom ON ligcom.numcom = entcom.numcom
WHERE YEAR(datcom) = annee AND entcom.numfou = codefournis;
