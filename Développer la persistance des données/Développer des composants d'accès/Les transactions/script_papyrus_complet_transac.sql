...
/* 1) Le dernier SELECT retourne la donnée modifiée mais comme l'instruction COMMIT n'a pas été exécutée, cette modification n'est pas validée.
   2) Les données ne sont pas modifiables par d'autres utilisateurs. 
   3&4) La transaction n'est pas terminée, il faut la validée en exécutant une instruction COMMIT.
   4) Il faut exécuter l'instruction ROLLBACK.

--Ex2 Les verrouillages

1) Ils ne peuvent pas y accéder car ils pourraient la perturber avant qu'elle soit exécutée.
2)
